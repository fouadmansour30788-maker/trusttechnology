...
import { refreshSearchTrending, refreshBestBuyTrending } from '@/lib/trending'
...
export async function GET(req: Request) {
  ...
  // Trending refreshes (Google Trends, Best Buy) piggyback on this cron (Vercel
  // Hobby caps us at 2 cron jobs, both already spoken for) — tight deadlines so
  // a slow fetch never risks the digest's own 60s budget; failures here don't
  ...
  const [ordersRes, pendingRes, lowRes, changesRes] = await Promise.all([
    ...
    refreshBestBuyTrending(Date.now() + 15_000),
  ])
