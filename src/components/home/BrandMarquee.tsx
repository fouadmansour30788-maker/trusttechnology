    ...
    <section className="py-10 border-y border-slate-200/70 bg-white/50">
      ...
      <div className="relative overflow-hidden [mask-image:linear-gradient(to_right,transparent,black_12%,black_88%,transparent)]">
        <div className="flex w-max items-center animate-marquee gap-14 px-7">
          {row.map((b, i) => (
            <div key={`${b.name}-${i}`} className="shrink-0 flex items-center h-14 select-none" title={b.name}>
              {b.logo ? (
                // eslint-disable-next-line @next/next/no-img-element
                <img
                  src={b.logo}
                  alt={b.name}
                  className="h-12 w-auto max-w-[170px] object-contain opacity-90 hover:opacity-100 hover:scale-105 transition-all duration-300"
                />
              ) : (
